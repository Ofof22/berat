 ...
 var polisler = await dbs.has(`soygun_${message.author.id}`)
  ...
  if(polisler === true) return message.reply(`Polisler \`Black marketi\` denetliyor burdan uzaklaşman lazım. \n**Not: polisler gidince black markete girebilirsin**`)
      ...
      if (args[0] === "bıçak") {
        if (para < 2000) {
          message.reply(
            "`Bıçak` almak için 2000₺ ye ihtiyacın var. Senin paran: **" +
              para +
              "**"
          );
        }
      ...
      if (args[0] === "pistol") {
        if (para < 2500) {
          message.reply(
            "`Pistol` almak için 2500₺ ye ihtiyacın var. Senin paran: **" +
              para +
              "**"
          );
        }
      ...
      if (args[0] === "uzi") {
        ...
        if (para > 3500) {
         ...
         await db.add(`${message.author.id}.uzi_adet`, 1);
          await db.set(`bakiye_${message.author.id}`, para - 3500);
          dbs.set(`blackmarket_${message.author.id}`, Date.now());
          client.channels.cache.get(soygunlog).send(`**${message.author.tag}**, Adlı kullanıcı black marketten \`Uzi\` satın aldı. Geriye **${para - 3500}**TL'si kaldı.`)
        }
      }
      ...
      if (args[0] === "ak-47") {
        ...
        if (para > 4000) {
          ...
          dbs.set(`blackmarket_${message.author.id}`, Date.now());
          client.channels.cache.get(soygunlog).send(`**${message.author.tag}**, Adlı kullanıcı black marketten \`Ak-47\` satın aldı. Geriye **${para - 4000}**TL'si kaldı.`)
        }
      }
