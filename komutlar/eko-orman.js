...
const taşbalta = db.get(`taşbalta_${message.author.id}`)
    ...
    if(!args[0]) return message.reply(`Bir maden belirtiniz. Örnek: b!orman ladin/koyu-meşe/meşe/huş`)
if(args[0] === "ladin") {
    if(!elmasbalta) return message.reply(`Hiç elmas kazmanız bulunmamaktadır! b!market`)
...
if(args[0] === "koyu-meşe") {
  if(!demirbalta) return message.reply(`Hiç demir kazmanız bulunmamaktadır! b!market`)
...
if(args[0] === "huş") {
      if(!taşbalta) return message.reply(`Hiç taş baltanız bulunmamaktadır! b!market`)
