...
let sure = await db.fetch(`calissüre_${message.member.id}`)
   ...
   if(!args[0]) return message.reply(`Bir maden belirtiniz. Örnek: b!maden zümrüt/elmas/altın/demir/taş`)
if(args[0] === "zümrüt") {
    if(!elmaskazma) return message.reply(`Hiç elmas kazmanız bulunmamaktadır! b!market`)
...
if(args[0] === "elmas") {
  if(!demirkazma) return message.reply(`Hiç demir kazmanız bulunmamaktadır! b!market`)
